...
def begin(gridworld, startx, starty, endx, endy):
    ...
    startNode = Node(endrow, endcolumn, 0, h, h + 0, "start")
    openheap.append(startNode)
    endNode = Node(startrow, startcolumn, math.inf, 0, math.inf, "closed")
    ...
    while openheap:
        ...
        # node is the closedlist returned form computepath
        node = computepath(openheap, closedlist, mentalworld, endNode)
        if len(closedlist) > 0 and closedlist[0].x != startx and closedlist[0].y != starty:
            print("not found")
            break
        # will find the lowest f cost path that leads to goal
        path = followDirections(node)
        # will be used to trace the traveled path that the agent takes
        traveled = []
        ...
        while path:
            ...
            # has reached goal from path
            if gridworld[currentblock.x][currentblock.y] == 3:
                print("done")
                break
            # has encountered an empty node
                ...
                g = 0
                endNode = Node(currentblock.x, currentblock.y, 0, h, h + 0, "start")
                traveled.append(endNode)
            # has encountered a blocked node
            else:
        ...
        startx = currentblock.x
        starty = currentblock.y
        #update position of agent in mentalworld
...
def computepath(openheap, closedlist, mentalworld, end):
    ...
    if closedlist[0] != end:
        ...
        return closedlist
    return closedlist
# Updates mentalworld when traversing the computedpath
...
# finds the path to traverse, if my team could have done anything this would have been a tree
