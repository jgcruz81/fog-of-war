...
def beginB(gridworld, startx, starty, endx, endy):
    # intialize mental world
    mentalworld = zeros([50, 50])
    mentalworld[startrow][startcolumn] = 2
    mentalworld[endrow][endcolumn] = 3
    ...
    while openheap:
        ...
        # node = the goal state
        node = computepathB(openheap, closedlist, mentalworld, endNode)
        totalexpanded = totalexpanded + len(closedlist)
...
def computepathB(openheap, closedlist, mentalworld, end):
    openMinFCost = []
    openheap = sorted(openheap, key=operator.attrgetter('f'))
    while openheap:
        # Looking for min F values then min H values if tied F values (3)
...
for x in range(50):
    ...
    start_time = time.time()
    beginB(gridworld, startrow, startcolumn, endrow, endcolumn)
    string = str(round(time.time() - start_time, 4))
    string = string + "\n"
    file1.write(string)
